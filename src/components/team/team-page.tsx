...
function DemoTeamLocked({ totalCount }: Readonly<{ totalCount: number }>) {
  return (
    <section className="mt-7 rounded-2xl border border-indigo-200 bg-indigo-50 p-6 shadow-sm">
      <p className="text-sm font-semibold text-indigo-700">Demo read-only</p>
      <h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">
        Team directory hidden
      </h2>
      <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
        This workspace has {totalCount} organization members. Names, email
        addresses, pending invitations, and access controls are hidden for the
        public demo account.
      </p>
    </section>
  );
...
export function TeamPage() {
  ...
  const isDemoUser = Boolean(
    membersQuery.data?.isDemoUser || invitationsQuery.data?.isDemoUser,
  );
  const isAdmin = isLoaded && membership?.role === "org:admin" && !isDemoUser;
  ...
  return (
    <>
      <section>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-600">
              {isDemoUser
                ? "Public demo mode hides member identities and access controls."
          ...
          {isAdmin ? (
            ...
            <span className="w-fit rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
              {isDemoUser ? "Demo read-only" : "View only"}
            </span>
          )}
        </div>
        ...
        {isDemoUser ? (
          <DemoTeamLocked totalCount={membersQuery.data?.totalCount ?? 0} />
        ) : (
          <>
            ...
            <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              {invitationsQuery.isPending ? (
                ...
                <div className="p-6 text-sm text-rose-700">
                  Invitations could not be loaded: {invitationsQuery.error.message}
                </div>
